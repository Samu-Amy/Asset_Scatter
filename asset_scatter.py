...
#        layout.operator("assetscatter.add_scatter_group", text="Scatter Group", icon="ADD")
        layout.operator("assetscatter.add_scatter_system",
                        text="Scatter System", icon="ADD")
...
#        layout.prop(AssetScatterProperties, "is_paint_mode", toggle=True, data=bpy.context.mode)
        ...
        rows = 2
        row = layout.row()
        row.template_list("ASSETSCATTER_UL_scatter_groups",
                          "", scn, "custom", scn, "custom_index", rows=rows)

        col = row.column(align=True)
        col.operator("assetscatter.add_scatter_group", text="", icon="ADD")
#        col.operator("custom.list_action", icon='ADD', text="").action = 'ADD'
        col.operator("custom.list_action", icon='REMOVE',
                     text="").action = 'REMOVE'
        col.separator()
        col.operator("custom.list_action", icon='TRIA_UP',
                     text="").action = 'UP'
        col.operator("custom.list_action", icon='TRIA_DOWN',
                     text="").action = 'DOWN'

        row = layout.row()
        col = row.column(align=True)
        row = col.row(align=True)
        row.operator("custom.add_viewport_selection",
                     icon="HAND")  # LINENUMBERS_OFF, ANIM
        row = col.row(align=True)
        row = col.row(align=True)
        # NON ELIMINA GLI OGGETTI DAL FILE (solo dalla scen
...
class ASSETSCATTER_OT_add_scatter_group(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for col in bpy.data.collections:
            # -- TODO: usa variabile (se cambia il nome, cambia anche nella ricerca qua)
            if col.name == "Asset Scatter - Scatter Groups":
                scatter_groups_collection = col
                scatter_groups_collection_exist = True

        if not scatter_groups_collection_exist:
            scatter_groups_collection = bpy.data.collections.new(
                "Asset Scatter - Scatter Groups")
            bpy.context.scene.collection.children.link(
                scatter_groups_collection)

        # Add Scatter Group Object
        bpy.ops.mesh.primitive_cube_add()
        scatter_group_object = bpy.context.active_object
        # -- TODO: rinomina in base al numero di oggetti già creati (se non richiede troppe risorse)
        scatter_group_object.name = "Scatter Group"
...
class ASSETSCATTER_OT_add_scatter_system(bpy.types.Operator):
    ...
    def execute(self, context):
        # TODO: Da modificare per la release
        file_path = "F:\Blender project\Gumroad-Blendermarket\Asset_Scatter.blend"
        inner_path = "NodeTree"
        ...
        if "Asset Scatter - Scatter Groups" not in bpy.data.node_groups:
            bpy.ops.wm.append(filepath=os.path.join(file_path, inner_path, node_name),
                              directory=os.path.join(file_path, inner_path), filename=node_name)
...
class ASSETSCATTER_OT_paint_vertex_group(bpy.types.Operator):
    ...
    def poll(cls, context):
        # -- TODO: controlla che l'oggetto abbia una modalità "weight paint"
...
class ASSETSCATTER_UL_scatter_groups(bpy.types.UIList):
    ...
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        layout.prop(item.obj, "name", text="", emboss=False,
                    translate=False, icon="MOD_PARTICLES")
...
def register():
    ...
    bpy.types.Scene.asset_scatter_pointer_properties = bpy.props.PointerProperty(
        type=AssetScatterProperties)
    bpy.types.Scene.asset_scatter_properties = CollectionProperty(
        type=ASSETSCATTER_PG_scatterGroupsCollection)
